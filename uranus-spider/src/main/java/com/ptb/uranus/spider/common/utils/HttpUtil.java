 ...
 * 这个工具类封装了常用的通过HTTP操作的一些工具 <br>
 * 1.包括获取一个新的HTTP prxoy地址 <br>
 * 2.Cookie转换工具 <br>
 * 3.
 */
public class HttpUtil {
    ...
    /**
     * The constant proxyServerAddress.
     * 去哪里请求代理服务地址
     */
    ...
    /**
     * Convert cookie as string map. <br>
     * 将一个网络传输中的标准Cookie字符串,转换成MAP的K,V形式
     *
    ...
    /**
     * Gets page by client.
     * 先尝试通过代理去访问一个地址,如果失败则通过本地去访问地址,共尝试3次,如果成功返回对应的RESPONSE对象,如果失败则抛出异常信息
     *
     ...
     */
    private static Response getPageByClient(String url, String ua, CookieStore cookieStore) throws IOException {
        return getPageByClient(url, ua, cookieStore, false);
    }

    private static Response getPageByClient(String url, String ua, CookieStore cookieStore, Boolean useProxy) throws IOException {
        int retryNum = TryNum;
        while (retryNum-- > 0) {
            try {
                try {
    ...
    public static String getPageSourceByClient(String url, String ua, CookieStore cookieStore, String charset, String nessaryString, boolean useProxy) {
        int retryNum = TryNum;
        while (retryNum-- > 0) {
            try {
                try {
    ...
    public static String getPageSourceByClient(String url, String ua, CookieStore cookieStore, String charset, String nessaryString) {
